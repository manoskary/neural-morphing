    ...
    with gr.Blocks(title="Neural Morphing Evaluation Dashboard") as demo:
        ...
        with gr.Tab("Evaluate"):
            ...
            with gr.Row():
                ...
                threshold = gr.Slider(0.2, 1.6, value=1.0, label="Threshold")
                continuity = gr.Slider(0.0, 1.0, value=0.3, label="Continuity")
                rvq_focus = gr.Slider(0.0, 1.0, value=0.5, label="RVQ Focus")
            with gr.Row():
                unit = gr.Number(label="Unit", value=2, precision=0)
                stride = gr.Number(label="Stride", value=2, precision=0)
                top_k = gr.Number(label="Top-K", value=4, precision=0)
