...
def test_server(base_url="http://localhost:8000"):
    ...
    try:
        ...
        print("\n2. Testing /encode endpoint (expecting error with invalid path)...")
        ...
        print("\n3. Testing /tokens_to_vectors endpoint...")
        response = requests.post(f"{base_url}/tokens_to_vectors",
                               json={"B": 1, "T": 1, "tokens": [42], "frame_index": 0},
                               timeout=10)
        ...
        print("\n4. Testing /decode endpoint...")
        response = requests.post(f"{base_url}/decode",
                               json={"B": 1, "T": 1, "tokens": [42]},
                               timeout=10)
        if response.status_code in [200, 500]:
